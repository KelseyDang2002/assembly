...
;At that instant the top of stack contains the resumption address where execution will resume when this assembly program returns to the caller.
...
;This file name: stackdump.asm
;This file language: X86
;This file syntax: Intel
;Assemble: nasm -f elf64 -o dump.o -l dump.lis stackdump.asm

;Program to display a contiguous segment of memory:
;Parameters passed in:
;     rdi = identifying integer
;     rsi = number of qwords below and outside the of stack, a positive integer
...
;    The zero-point is a location (or an address) where the human user of this software has focused attention.  This quantity is passed in by the calling program.

;Prototype of this function:
;    unsigned long dumpstack(long a, unsigned long b, unsigned long c, unsigned long * d);
...
;Lessons to learn: What is an address? Your first answer is probably 64 bits holding a non-negative integer.  That is true, and we call such things "unsigned longs".
;However, an address is rather special kind of "unsigned long".  For example we never think of multiplying together two unsigned longs.  Nor do we ever consider
;dividing one address by another address to obtain a third address.  Therefore, we come to the question of what data type in C++ best describes 64 numbers with a
;limited set of arithmetic operations allowed?  The answer has to be (unsigned long *), or as we say "pointer to unsigned long data.

;Clarification of term: In the description of how this program works you will find strings such as "current position of rsp".  That means the position of rsp at
;the time this subprogram, dumpstack, was called.  The reader of software like this one should be aware of this sutile distinction.

extern printf
extern showgprs                                             ;Only used for debugging purposes.  Should be removed later.
global dumpstack                                            ;This declaration allows the subprogram to be called from outside this file.
...
dumpstack:                                                  ;Where execution begins when this program is called.
...
mov qword r12,[rsp+10*8]                                    ;Copy the new zero point to r12
